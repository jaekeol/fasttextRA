...
void Dictionary::computeSubwords_only(
    const std::string& word,
    std::vector<int32_t>& ngrams,
    std::vector<std::string>* substrings, int maxn, int minn) const {
  ...
  for (size_t i = 0; i < size_; i++) {
	...
	if(word == "<the>"){
		...
		for(int j=0; j<words_[i].subwords.size(); j++){
			std::cout << "!!!" << words_[i].subwords[j] << std::endl;
		}
	}
	*/
  }
...
int32_t Dictionary::getNgramId(const std::string& ngram){
  int32_t id = hash(ngram) % args_->bucket;
  if (pruneidx_size_ > 0) {
    if (pruneidx_.count(id)) {
      id = pruneidx_.at(id);
    }
