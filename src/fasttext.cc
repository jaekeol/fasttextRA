...
void FastText::addInputVector(Vector& vec, int32_t ind, float a) const {
  if (quant_) {
    vec.addRow(*qinput_, ind);
  } else {
    vec.addRow(*input_, ind, (real)a);
  }
}
...
void FastText::getWordVector_w(Vector& vec, const std::string& word) {
  const std::vector<int32_t>& ngrams = dict_->getSubwords(word);
  vec.zero();
  float w;
  float wsum = 0;
  ...
  for (int i = 0; i < ngrams.size(); i++) {
    ngram = ngrams[i];
	if(idf_.find(ngram) != idf_.end()){
		w = idf_[ngram];
	}else{
		w = 1.0;
	}
    addInputVector(vec, ngrams[i], w);
	wsum = wsum + w;
  }
  if (ngrams.size() > 0) {
    //vec.mul(1.0 / ngrams.size());
...
void FastText::skipgram(
  ...
  for (int32_t w = 0; w < line.size(); w++) {
	...
	getWeight(ngrams, weights);
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line.size()) {
        model.update(ngrams, weights, line, w + c, lr);
      }
    }
  }
}

void FastText::getWeight(const std::vector<int32_t>& ngrams, std::vector<float>& weights){
	int32_t ngram;
	float w;
...
void FastText::readIdf(const std::string& filename){
  std::ifstream ifs(filename);
  if (!ifs.is_open()) {
    throw std::invalid_argument(
        args_->input + " cannot be opened for reading idf!");
  }
  ...
  while(ifs.getline(line, 2048)){
	...
	// get id
	wid = dict_->getNgramId(ngram);

	//
	if(wid >= 0){
		idf_[wid] = atof(value);
		//std::cout << ngram << "\t" << wid << "\t" << value << std::endl;
		ngram_cnt++;
	}else{
		std::cout << "can't find id : " << ngram << std::endl;
	}	
  }
  std::cerr << ngram_cnt << " ngram read" << std::endl;
